...
public class PayAndTransformToOrderCommandHandler implements CommandHandler<PayAndTransformToOrderCommand, CommandResponse<Event>> {
    ...
    public CommandResponse<Event> handle(PayAndTransformToOrderCommand command) {
        var response = payCommandHandler.handle(new PayCommand(command.cartId(), command.cardNumber(), command.expirationDate(), command.cypher(), command.amount(), command.cartType()));
        ...
        return transformToOrderCommandHandler.handle(new TransformToOrderCommand(transactionId, command.cartId(), command.amount(), command.cartType()));
    }
