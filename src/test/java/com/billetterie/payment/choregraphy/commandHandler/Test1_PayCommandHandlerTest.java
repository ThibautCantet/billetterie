...
public class Test1_PayCommandHandlerTest {
    ...
    class Handle {
        ...
        void should_return_ValidationRequested_when_pending_transaction() {
            var command = new PayCommand(CART_ID, CARD_NUMBER, EXPIRATION_DATE, CYPHER, AMOUNT, EMAIL);
            var transaction = new Transaction(TRANSACTION_ID, PaymentStatus.PENDING, REDIRECTION_URL);
            var payment = new Payment(CART_ID, CARD_NUMBER, EXPIRATION_DATE, CYPHER, AMOUNT, EMAIL);
